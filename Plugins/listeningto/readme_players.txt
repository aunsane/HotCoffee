...
- Winamp (32bit):
  copy the gen_mlt.dll to <WinampDir>\Plugins\gen_mlt.dll

- foobar2000 (32bit):
  for full listeningto support install comserver2 inside foobar.
  http://wiki.hydrogenaudio.org/index.php?title=Foobar2000:Components_0.9/COM_Automation_Server_(foo_comserver2)
  alternativ copy the foo_mlt.dll to <FoobarpluginsDir>\components\foo_mlt.dll
  foo_mlt is not need if user install comserver2 inside foobar
  if both plugins present inside foobar, foo_mlt is not active
